...
if [[ "$@" == "" ]]; then
    PYTHONPATH=. pyinstaller --clean -y --icon zeus.ico --noconsole --onefile --name "ZeusTrusteeClient" zeus_client_gui.py
    PYTHONPATH=. pyinstaller --clean -y --icon zeus.ico --onefile --name "ZeusTrusteeClientConsole" zeus_client_gui.py
